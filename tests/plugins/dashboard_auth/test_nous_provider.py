...
class TestVerifySession:
    ...
    def test_expired_token_returns_none(self, provider, rsa_keypair):
        token = _mint_token(rsa_keypair, ttl_seconds=-120)
        assert provider.verify_session(access_token=token) is None

    def test_clock_skew_within_one_minute_is_accepted(self, provider, rsa_keypair):
        token = _mint_token(rsa_keypair, ttl_seconds=-30)
