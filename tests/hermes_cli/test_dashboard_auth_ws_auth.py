...
class TestWsAuthOkGated:
    """Gate ON — remote tickets and loopback-only desktop process tokens."""

    @pytest.mark.parametrize("peer", ["127.0.0.1", "::1", "::ffff:127.0.0.1"])
    def test_local_process_token_stamps_identity(self, gated_app, peer):
        ws = _fake_ws(query={"token": web_server._SESSION_TOKEN}, client_host=peer)
        assert _web_server_chat._ws_auth_ok(ws)
        assert ws._hermes_auth_identity == {"user_id": "loopback-admin", "provider": "local"}

    @pytest.mark.parametrize("peer", ["192.0.2.1", "testclient", "", "localhost"])
    def test_forwarded_loopback_cannot_authorize_process_token(self, gated_app, peer):
        ws = _fake_ws(query={"token": web_server._SESSION_TOKEN}, client_host=peer)
        ws.headers["x-forwarded-for"] = "127.0.0.1"
        assert not _web_server_chat._ws_auth_ok(ws)

    def test_missing_peer_rejects_process_token(self, gated_app):
        ws = _fake_ws(query={"token": web_server._SESSION_TOKEN})
        ws.client = None
        assert not _web_server_chat._ws_auth_ok(ws)
    ...
    def test_legacy_token_rejected_for_remote_peer_in_gated_mode(self, gated_app):
