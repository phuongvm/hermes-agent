...
def _validate_openspec_transition(conn: sqlite3.Connection, task_id: str) -> None:
    meta_exists = conn.execute("SELECT 1 FROM sqlite_master WHERE type='table' AND name='openspec_enforcement_meta'").fetchone()
    if not meta_exists:
        return
    active = conn.execute("SELECT value FROM openspec_enforcement_meta WHERE key='enforcement_status'").fetchone()
    if not active or active[0] != "active":
        return
    row = conn.execute("SELECT openspec_contract, openspec_contract_hash FROM tasks WHERE id=?", (task_id,)).fetchone()
    if row is None:
        raise ValueError(f"Task {task_id} not found")
    if row[0] and not row[1]:
        raise ValueError("OpenSpec contract hash is required for a contracted task")
...
def complete_task(
    ...
    if acceptance is False:
        return False
    with write_txn(conn):
        _validate_openspec_transition(conn, task_id)
...
def record_task_identity_event(task_id: str, identity: dict, event_type: str) -> bool:
    from hermes_cli import kanban_db_connect
    db_path = kanban_db_path()
    if not db_path.is_file():
        return False
    with kanban_db_connect.connect_closing(db_path) as conn:
        with write_txn(conn):
            row = conn.execute("SELECT current_run_id FROM tasks WHERE id=?", (task_id,)).fetchone()
            if row is None or row[0] is None:
                return False
            conn.execute(
                "INSERT INTO task_run_identity_events "
                "(id, run_id, task_id, identity_snapshot, event_type, created_at) VALUES (?, ?, ?, ?, ?, ?)",
                ("evt_" + __import__("uuid").uuid4().hex, row[0], task_id, json.dumps(identity), event_type, time.time()))
            return True
