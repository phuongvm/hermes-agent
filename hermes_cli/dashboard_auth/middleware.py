...
def _session_expired_response(request: Request) -> Response:
    ...
    audit_log(AuditEvent.SESSION_VERIFY_FAILURE, reason="no_provider_recognises",
              ip=_client_ip(request))
    response = _auto_sso_response(request, unauth_reason="invalid_or_expired_session")
    if response is None:
...
async def gated_auth_middleware(
    ...
    from hermes_cli.dashboard_auth.request_utils import is_loopback_process_token
    from hermes_cli.web_server import _SESSION_TOKEN
    peer_host = request.client.host if request.client else ""
    local_token = bearer or request.headers.get("X-Hermes-Session-Token", "")
    if is_loopback_process_token(peer_host, local_token, _SESSION_TOKEN):
        request.state.session = Session(
            user_id="loopback-admin", email="admin@local", display_name="Admin", org_id="",
            provider="local", expires_at=2147483647, access_token=local_token, refresh_token="")
        return await call_next(request)
    if bearer:
        try:
