...
from hermes_cli.dashboard_auth.cookies import (
    ...
    set_session_cookies)
from hermes_cli.dashboard_auth.login_page import render_login_html, render_native_provider_choice_html
...
async def auth_native_authorize(
    ...
    if not provider:
        native_eligible = [candidate for candidate in list_session_providers()
                           if not getattr(candidate, "supports_password", False)]
        if len(native_eligible) > 1:
            return HTMLResponse(render_native_provider_choice_html(
                providers=native_eligible, authorize_path=f"{_prefix(request)}/auth/native/authorize",
                code_challenge=code_challenge, code_challenge_method=code_challenge_method,
                redirect_uri=redirect_uri, state=state), headers={"Cache-Control": "no-store, no-cache, must-revalidate"})
