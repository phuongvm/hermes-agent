...
async def fs_list(path: str):
    ...
    try:
        ...
        insensitive = os.name == "nt" or sys.platform == "darwin"
        hidden_dirs = {name.lower() for name in hidden_dirs} if insensitive else hidden_dirs
        with os.scandir(target) as scan:
            for entry in scan:
                if ((entry.name.lower() if insensitive else entry.name) in hidden_dirs
                        or _is_sensitive_path(Path(entry.path))):
                    continue
                entries.append({
                    "name": entry.name,
