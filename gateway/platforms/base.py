...
async def _cache_media_from_url(url: str, ext: str, retries: int, *, media_type: str, accept: str,
                                cache_fn, log_label: str, headers: Optional[Dict[str, str]] = None) -> str:
    ...
    headers = {"User-Agent": "Mozilla/5.0 (compatible; HermesAgent/1.0)", "Accept": accept, **(headers or {})}
...
async def cache_image_from_url(url: str, ext: str = ".jpg", retries: int = 2,
    ...
    return await _cache_media_from_url(
        ...
        cache_fn=cache_image_from_bytes, log_label="Image", headers=headers)
            ...
            try:
                ...
                if image_url.startswith("file://"):
                    ...
                    if parsed.netloc and parsed.netloc.lower() != "localhost":
                        uri_path = (f"{parsed.netloc}{parsed.path}" if len(parsed.netloc) == 2
                                    and parsed.netloc[1] == ":" else f"//{parsed.netloc}{parsed.path}")
                    sender, url_kw = self.send_image_file, {"image_path": url2pathname(uri_path)}
