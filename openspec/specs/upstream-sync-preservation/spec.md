# Specification: Upstream Sync Preservation

## Purpose
Governs the evidence-gated reconciliation of upstream changes into the local Hermes Agent repository. Establishes the non-negotiable contract for preserving critical local invariants across authentication, reconnect resilience, WebSocket transport tuning, dependency reproducibility, and multi-agent verification.
...
- **THEN** the workspace and branch MUST be the authorized integration worktree `.worktrees/sync-upstream-main-v2` and `sync/upstream-main-v2`
- **AND** local baseline `43ca20a5fd` and upstream target `c661785f87` MUST match the recorded revisions or receive a documented scope update before merge
...
The integration SHALL retain a resolution ledger covering all 10 content conflict paths observed at the pinned revisions, dependency manifests and locks, and invariant-critical auto-merged dependencies. An absent or moved path SHALL be explicitly accounted for rather than silently omitted.

#### Scenario: Conflict catalog completeness
- **WHEN** merge-tree analysis is performed between local baseline `43ca20a5fd` and upstream `c661785f87`
- **THEN** all 10 content conflict files (`acp_adapter/session.py`, `apps/desktop/electron/main.ts`, `apps/desktop/scripts/set-exe-identity.mjs`, `apps/desktop/src/app/right-sidebar/files/use-project-tree.test.ts`, `apps/desktop/src/app/right-sidebar/files/use-project-tree.ts`, `apps/desktop/src/app/session/hooks/use-model-controls.test.tsx`, `plugins/platforms/buzz/adapter.py`, `tests/hermes_cli/test_mcp_startup.py`, `tui_gateway/methods_prompt.py`, `uv.lock`) MUST be tracked in `resolution-ledger.md`
- **AND** each conflict MUST receive a documented resolution strategy and test evidence mapping to its respective invariant or component contract
...
- **AND** profile refreshes, project-tree directory reads (`loadRoot`/`loadChildren`), and background polling MUST stop without scheduling retry timers
...
- **AND** suspended work (including project tree synchronization via `registerResumeSyncHandler`) MUST resume once per data source rather than as a request cascade
...
- **AND** the existing consecutive-rejection debounce policy (at least two 401s within 15 seconds) MUST remain separate from immediate terminal suspension
...
- **AND** configured keepalive/open timeouts (`_DEFAULT_WS_OPEN_TIMEOUT = 30.0`, `_DEFAULT_WS_PING_INTERVAL = 30.0`, `_DEFAULT_WS_PING_TIMEOUT = 60.0`) MUST reach the actual WebSocket connection
