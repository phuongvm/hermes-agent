# Specification: Desktop Runtime Version Resolution

## Purpose
Defines the authoritative version resolution ladder consumed by the Hermes Desktop main process (`hermes:version` IPC, About dialog, and remote skew detection). Resolves whether the running desktop client reports its bundled build stamp or falls back to local source checkouts based on packaging mode.
