...
- [CODER] 15:18: Completed Milestone 5 (t_1ebcd53e) for change `desktop-reconnect-session-resilience`. Implemented debounced reauth latch and two-tier 401 classifier in `apps/desktop/electron/backend-health.ts`: track consecutive 401 count and timestamps per endpoint, require >=2 consecutive 401s within 15s from previous 'open' gateway state before escalating to isReauthRequired, reset counter on non-401 responses. Verified with vitest (33/33 tests in backend-health.test.ts, 33/33 passing in related desktop stores/hooks).
- [CODER] 15:45: Completed Milestone 6 (t_6ec1c44f) for change `desktop-reconnect-session-resilience`. Implemented single global reauth modal with process-wide singleton latch `ReauthModalLatch` in `apps/desktop/electron/reauth-modal-latch.ts` and integrated into Electron main auth handler (`ipcMain.handle('hermes:connection-config:oauth-login')` in `apps/desktop/electron/main.ts`). When multiple pooled connections trigger reauth simultaneously, exactly one modal opens; concurrent triggers are queued and resolve their auth state upon successful reauth. Verified with 9 new unit tests in `reauth-modal-latch.test.ts` (42/42 total passing in electron suite, 70/70 passing across related desktop tests) and strict OpenSpec validation.
