...
class TurnFacadeMixin:
    ...
    def run_conversation(
        ...
        try:
            ...
            try:
                from hermes_cli.kanban_db import record_task_identity_event
                record_task_identity_event(effective_task_id, {
                    "requested_model": getattr(self, "model", "") or "",
                    "requested_provider": getattr(self, "provider", "") or "",
                    "effective_model": getattr(self, "_active_model", self.model) or "",
                    "effective_provider": getattr(self, "_active_provider", self.provider) or "",
                }, "task_start")
            except Exception as event_error:
                logger.warning("Could not record task-start identity event: %s", event_error)
