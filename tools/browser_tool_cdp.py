...
def _resolve_cdp_override(cdp_url: str) -> str:
    ...
    if lowered.startswith(("ws://", "wss://")):
        if not (discovery_url.count(":") == 2 and discovery_url.rsplit(":", 1)[-1].isdigit() and "/" not in discovery_url.split(":", 2)[-1]):
            return raw
...
def _maybe_start_user_session_cdp() -> None:
    ...
    try:
        ...
        _origin().logger.warning("Automatic user-session CDP startup failed: %s", exc)
