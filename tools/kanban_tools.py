...
def _visible(*, to_env_worker: bool) -> bool:
    ...
    try:
        ...
        row = None
        if kanban_db_path().is_file():
            with kanban_db_connect.connect_closing() as conn:
                row = conn.execute("SELECT value FROM openspec_enforcement_meta WHERE key='enforcement_status'").fetchone()
        if row and row[0] == "active" and get_active_profile_name() not in AUTHORIZED_ACTIVATORS:
            return False
    except Exception:
        return False
