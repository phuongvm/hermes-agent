...
// Terminal signed-out latch (local invariant): once a host's native session is
// authoritatively rejected (refresh 401) or explicitly logged out, stop minting
// tokens for it until a confirmed login clears the latch. The renderer mirrors
// this via 'hermes:auth:terminal-state-changed'.
const terminalSignedOutHosts = new Map<string, string>()

function isTerminalSignedOut(baseUrl: string): boolean {
  return terminalSignedOutHosts.has(normalizeRemoteBaseUrl(baseUrl))
}

function markTerminalSignedOut(baseUrl: string, reason = 'terminal_rejection'): void {
  const key = normalizeRemoteBaseUrl(baseUrl)
  terminalSignedOutHosts.set(key, reason)
  broadcastAuthTerminalStateChanged({ baseUrl: key, signedOut: true, reason })
}

function clearTerminalSignedOut(baseUrl: string): void {
  const key = normalizeRemoteBaseUrl(baseUrl)

  if (terminalSignedOutHosts.delete(key)) {
    broadcastAuthTerminalStateChanged({ baseUrl: key, signedOut: false })
  }
}

interface EnsureNativeAccessTokenOptions extends NativeAccessTokenOptions {
  /** Alias of forceRefresh (local single-replay call sites). */
  force?: boolean
  /** Alias of rejectedAccessToken (local single-replay call sites). */
...
// there are no tokens, the host is terminally signed out, or the refresh is
// terminally rejected (caller re-logins). A forced refresh that ends with the
// stored tokens cleared is an authoritative rejection and latches the host.
async function ensureNativeAccessToken(
  baseUrl: string,
  options: EnsureNativeAccessTokenOptions = {}
): Promise<string | null> {
  if (isTerminalSignedOut(baseUrl)) {
    return null
  }

  const forceRefresh = Boolean(options.forceRefresh || options.force)
  const rejectedAccessToken = options.rejectedAccessToken || options.rejectedBearer
  const hadTokens = _loadNativeTokens(baseUrl) !== null
  const accessToken = await nativeAccessTokenCoordinator.ensure(baseUrl, { forceRefresh, rejectedAccessToken })

  if (accessToken === null && hadTokens && _loadNativeTokens(baseUrl) === null) {
...
ipcMain.handle('hermes:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
  const nativeTokens = _loadNativeTokens(baseUrl)

  // Revoke the server-side native session first (best effort); an older
  // gateway without /auth/native/logout 404s and is ignored.
  if (nativeTokens?.refreshToken) {
    try {
      await postJsonNoAuth(
        nativeRefreshUrl(baseUrl).replace(/\/refresh$/, '/logout'),
        { refresh_token: nativeTokens.refreshToken, provider: nativeTokens.provider },
        { timeoutMs: 10_000 }
      )
    } catch (error: any) {
      if (readStatusCode(error) !== 404) {
        rememberLog('[native-oauth] Server session revocation failed; clearing local credentials.')
      }
    }
  }
  ...
  nativeAccessTokenCoordinator.clearTokens(baseUrl)
  markTerminalSignedOut(baseUrl, 'explicit_logout')
