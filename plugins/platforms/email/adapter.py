...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if os.getenv("EMAIL_DISABLE_AUTO_REPLY", "").lower() in {"1", "true", "yes"}:
            logger.info("Email adapter paused: automatic polling and sending are disabled")
            return True
    ...
    def _smtp_send(self, msg: MIMEMultipart) -> None:
        ...
        if os.getenv("EMAIL_DISABLE_AUTO_REPLY", "").lower() in {"1", "true", "yes"}:
            raise RuntimeError("Automatic email sending is disabled by EMAIL_DISABLE_AUTO_REPLY")
        smtp = self._connect_smtp()
        try:
            smtp.login(self._address, self._password)
